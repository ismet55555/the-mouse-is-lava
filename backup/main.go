...
func init() {
	...
	configs := Configs{}
	v1.Unmarshal(&configs)

	// Load messages
	v2 := viper.New()
	v2.SetConfigName("messages")
	v2.AddConfigPath("./config")
	v2.SetConfigType("yaml")
	if err := v2.ReadInConfig(); err != nil {
		panic(fmt.Errorf("Failed reading config file: %w", err))
	}
	v2.AllSettings()
	messages := Messages{v2.GetStringSlice("encouragements"), v2.GetStringSlice("insults")}
	fmt.Println(messages.encouragements)
}
